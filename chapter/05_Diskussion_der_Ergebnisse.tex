...
\begin{table}[htbp]
  \centering
  \small
  \caption{Strukturierte Übersicht der vier identifizierten morphologischen Forschungslücken}
  \label{tab:morphologische_luecken}
  \begin{tabularx}{\textwidth}{cl>{\raggedright\arraybackslash}p{4.4cm}>{\raggedright\arraybackslash}X}
    \toprule
    \textbf{Nr.} & \textbf{Kombination} & \textbf{Strukturelles Defizit in der Literatur} & \textbf{Prioritäre Forschungsagenda} \\
    \midrule
    \textbf{1} & $(v_{2.3} \times v_{6.2})$ DSS $\times$ ISO & Fehlen interaktiver DSS mit ISO 25010 & Quelloffene modulare DSS-Plattformen \\
    \textbf{2} & $(v_{1.3} \times v_{3.4})$ OR $\times$ Glue-Code & Inkompatibilitäten nur binäre Restriktionen & Ko-Optimierung mit Wrapper-Kosten \\
    \textbf{3} & $(v_{2.3} \times v_{4.4})$ DSS $\times$ Evidential & Mangel an GUIs für Nichtwissensintervalle & Webbasierte Evidenz-DSS (Bel/Pl) \\
    \textbf{4} & $(v_{5.2} \times v_{6.2} \times v_{2.3})$ Benchmarking & Entkopplung von Qualitätsnorm und Testbeds & Integration von CI/CD, Fuzzing und CVE \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[htbp]
  \centering
  \small
  \caption{Kontingenz-Entscheidungsmatrix zur Methodenauswahl in der Praxis}
  \label{tab:entscheidungsmatrix_praxis}
  \begin{tabularx}{\textwidth}{>{\bfseries\raggedright\arraybackslash}p{3.2cm}>{\raggedright\arraybackslash}p{2.6cm}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}p{3.6cm}}
    \toprule
    Entscheidungskontext & Empfohlener Archetyp & Primäre Methoden & Zentraler Umsetzungshinweis \\
    \midrule
    Monolithische Systeme & Archetyp 1 \& 3 & Fuzzy-AHP / F-TOPSIS & Kriterienhierarchie schlank halten \\
    Modulare Cloud/CBSD & Archetyp 2 & Multikriterielle 0-1-Optimierung & Dynamische Pareto-Exploration \\
    Sicherheitskritisch & Archetyp 4 & Bayes-Netze \& Evidential Reasoning & Veto-Schwellen \& Benchmarking \\
    \bottomrule
  \end{tabularx}
\end{table}
