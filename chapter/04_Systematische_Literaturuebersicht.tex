...
\begin{table}[p]
  ...
  \begin{tabularx}{\linewidth}{l>{\raggedright\arraybackslash}p{4.2cm}cc>{\raggedright\arraybackslash}X}
    \toprule
    \textbf{Kategorie} & \textbf{Ausprägung / Dimension} & \textbf{Anzahl ($N$)} & \textbf{Anteil (\%)} & \textbf{Charakteristische Publikationen (Auswahl)} \\
    \midrule
    ...
    \multirow{3}{*}{\textbf{Publikationsorgan}} 
      & Internationale Fachzeitschriften & 21 & 53,8\,\% & \textcite{FCYW9EWJ}, \textcite{8VHUMEKY} \\
    ...
    \multirow{3}{*}{\textbf{Quality Appraisal}} 
      & Hohe Evidenzstärke & 36 & 92,3\,\% & \textcite{M3B2D3GD}, \textcite{QDUNCXFC} \\
      & Mittelere Evidenzstärke & 2 & 5,1\,\% & \textcite{DFASRUVC}, \textcite{LPC92LAP} \\
      & Niedrige Evidenzstärke & 1 & 2,6\,\% & \textcite{CZZ6XQZJ} \\
    \midrule
    \multicolumn{5}{p{\dimexpr\linewidth-2\tabcolsep\relax}}{\textbf{Mittelwerte der Qualitätsdimensionen (Skala 0,00 bis 2,00; Gesamtmittelwert: 6,87 / 8,00):} \newline
...
Zur Beantwortung von Forschungsfrage 2 (\textit{Welche methodischen Merkmale, Stärken und praktischen Grenzen kennzeichnen aktuelle Auswahlmethoden, und welche übergeordneten Lösungsarchetypen lassen sich identifizieren?}) wird die in Abschnitt~\ref{subsec:morphologische_analyse_methodik} hergeleitete General Morphological Analysis (GMA) nach Zwicky und Ritchey \parencite[S.~81--91]{Z6N6V5L9} empirisch auf das Primärliteraturkorpus angewendet.

\subsubsection{Induktive Konstruktion des morphologischen Feldes}
\label{subsubsec:feldkonstruktion}

Die sechs Schlüsselparameter ($P_1 \dots P_6$) und ihre diskreten Ausprägungen wurden induktiv und theoriegeleitet aus den Merkmalsclustern der 18-spaltigen Datenextraktionsmatrix (Abschnitt~\ref{subsec:qualitaetsbewertung_datenextraktion}) abgeleitet. Sie erfüllen die formalen Bedingungen der Parameter-Orthogonalität (wechselseitiger Ausschluss) und kollektiven Erschöpfung \parencite[S.~82--83]{Z6N6V5L9}:
...
\end{equation}
auf. Das vollständige morphologische Feld samt den relativen Häufigkeitsverteilungen im Primärkorpus ist in Tabelle~\ref{tab:morphologisches_feld} dargestellt.

Im Rahmen des durchgeführten Cross-Consistency Assessments (CCA) wurden alle $\binom{6}{2} = 15$ zweidimensionalen Sub-Matrizen mit insgesamt 219 paarweisen Zustandskombinationen evaluiert: 154 Kombinationen wurden im Schrifttum empirisch beobachtet ($O$), 61 Kombinationen stellen plausible und theoretisch konsistente Innovationsräume dar ($?$), während 4 Kombinationen als logisch-strukturell inkonsistent identifiziert wurden ($X$, z.\,B. rein manuelle Durchführung gepaart mit nicht-linearer Ganzzahloptimierung).
