...
Die operative Schnittstelle zwischen der Literaturübersicht und der Allgemeinen Morphologischen Analyse bildet die Datenextraktions- und Synthesethase. Während die SLR primär der empirischen Materialsammlung, Identifikation, Filterung und qualitativen Aufbereitung der Primärstudien dient, fungiert die GMA als analytisches Instrumentarium der Synthesestufe. Durch die Abbildung der extrahierten Methodeneigenschaften auf ein sechsgliedriges morphologisches Feld gelingt es, den theoretischen Raum aller denkbaren COTS-Auswahlarchitekturen systematisch aufzuspannen und mittels \textit{Cross-Consistency Assessment} (CCA) logische, empirische sowie methodische Inkompatibilitäten innerhalb des Forschungsstands aufzudecken.

In Tabelle~\ref{tab:methodenintegration} ist die funktionale Synthese und begriffliche Abbildung zwischen den acht Phasen nach \cite{2NHFUF5N}, den PRISMA 2020-Prozessstufen (\cite{BP42WGEK}) und den Phasen der Allgemeinen Morphologischen Analyse (\cite{Z6N6V5L9}) zusammenfassend dargestellt.

\begin{table}[htbp]
\centering
\small
\caption{Integration von Okoli (2015), PRISMA 2020 und Morphologischer Analyse (GMA)}
\label{tab:methodenintegration}
\begin{tabularx}{\textwidth}{X p{3.8cm} p{4.2cm} X}
\toprule
\textbf{Phase nach Okoli (2015)} & \textbf{PRISMA 2020 Stufe} & \textbf{Analytischer Fokus \& Werkzeuge} & \textbf{Transformation zur GMA} \\
\midrule
1. Purpose Identification & Protocol \& Registration & Definition der Forschungsfragen (FF1--FF3) \& Ziele (FZ1--FZ3) & Abgrenzung des untersuchten Problemraums \\
2. Protocol Drafting & Eligibility Criteria & Erstellung Suchstring, Einschluss- / Ausschlussregeln & Festlegung der Analyseparameter \\
3. Search Literature & Identification & Abfrage von IEEE, ACM, EBSCO, Springer & Identifikation des Primärdatenpools \\
4. Practical Screen & Screening (Title/Abs/Full) & Dreistufiges Filtern relevanter COTS-Auswahlmethoden & Bereinigung um irrelevanten Kontext \\
5. Quality Appraisal & Quality Assessment & Vierdimensionales QA-Schema (0--8 Punkte) & Exklusion valider Störgrößen \\
6. Data Extraction & Data Collection & 18-spaltige strukturierte Extraktionsmatrix & Befüllung des Parameterraums ($P_1$--$P_6$) \\
7. Data Synthesis & Synthesis of Results & Qualitative Codierung \& Morphologisches Feld & Durchführung des CCA \& Raumreduktion \\
8. Write Review & Reporting / Flow Diagram & PRISMA-Diagramm \& Morphologische Rekonstruktion & Interpretation der Lösungsräume \\
\bottomrule
\end{tabularx}
\end{table}
